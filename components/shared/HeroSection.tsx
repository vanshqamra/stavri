...
export const HeroSection = ({ title, description, primaryAction, secondaryAction }: HeroProps) => (
  <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-amber-900 py-24 text-white">
    ...
    <div className="absolute inset-0 bg-gradient-to-tr from-slate-900/70 via-amber-900/40 to-transparent" />
    <div className="relative mx-auto grid max-w-6xl gap-12 px-4 md:grid-cols-2">
      <div>
        <p className="text-sm uppercase tracking-[0.5em] text-amber-300">Rab Noor Pvt Ltd</p>
        <h1 className="mt-4 text-4xl font-semibold leading-tight md:text-5xl">{title}</h1>
        <p className="mt-6 text-lg text-amber-100">{description}</p>
        <div className="mt-8 flex flex-wrap gap-4 text-sm font-semibold">
          <Link
            href={primaryAction.href}
            className="rounded-full bg-amber-500 px-6 py-3 text-white shadow-xl shadow-amber-500/40"
          >
            {primaryAction.label}
          </Link>
          {secondaryAction && (
      ...
      <div className="rounded-3xl border border-white/20 bg-white/10 p-8 backdrop-blur">
        <p className="text-sm uppercase tracking-[0.3em] text-amber-200">Global Presence</p>
